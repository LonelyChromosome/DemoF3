...
class ScheduleDifferenceSheet extends StatelessWidget {
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
            ...
            Expanded(child: ListView(children: <Widget>[
              if (difference == null || difference.initial || !difference.hasChanges)
                const ListTile(title: Text('Chưa có thay đổi so với lần đồng bộ trước.'))
              else ...<Widget>[
                ...
                for (final name in difference.removedSubjects)
                  ListTile(leading: const Icon(Icons.remove_circle_outline),
  ...
  Widget _section(String title, ScheduleDifference data) => Column(
    ...
    children: <Widget>[
      ...
      Text('$title · ${data.added} mới, ${data.modified} đổi, ${data.removed} hủy',
        style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
      if (data.details.isEmpty)
        ListTile(title: Text(data.hasChanges
      ...
      for (final item in data.details)
        Card(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
            Text('${switch (item.kind) { 'added' => 'Mới', 'removed' => 'Đã hủy', _ => 'Đã đổi' }} · '
  ...
  String _row(ScheduleRecord row) {
    ...
    final time = '${two(start.hour)}:${two(start.minute)}–${two(end.hour)}:${two(end.minute)}';
